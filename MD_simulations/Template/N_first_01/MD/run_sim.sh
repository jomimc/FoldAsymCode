...
for i in {1..58} ; do
  ...
  if [ $i == 1 ] ; then
    python $INIT/protein_plus_wall.py --mode update  --top reference.top --gro reference.gro -i $i
  elif [ $i == 58 ] ; then
    python $INIT/protein_plus_wall.py --mode update  --top reference.top --gro confout.gro -i $i --free
    cp reference.top gromacs.top
  else
    python $INIT/protein_plus_wall.py --mode update  --top reference.top --gro confout.gro -i $i
  fi
